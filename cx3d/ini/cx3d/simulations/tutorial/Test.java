...
public class Test implements LocalBiologyModule {
	...
	public void run() {
		if (resource > 1) {
			resource--;
			System.out.println("Look at me now running and doing stuff");
			double speed = 100;
			double probabilityToBifurcate = 0.05; // o.oo5		
			...
			if(ECM.getRandomDouble()<probabilityToBifurcate){
				...
				nn[0].addLocalBiologyModule(new Test((int) resource/2));
				nn[1].addLocalBiologyModule(new Test((int) resource/2));
			}
		} else {
	...
	public static void main(String[] args) {
		...
		neurite.addLocalBiologyModule(new Test(1000));
