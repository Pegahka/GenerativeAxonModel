...
public class P23Side23 extends AbstractGrowthCone {
	...
	public void run() {
		...
		if(LSmaler(readCount(readTasteSuroundingSomasCellType(CtxConstants.L23)),val(1)).eval()){
		...
		if(LAnd(LSmaler(readMorphDiameter(),val(0.9)), Mthrowdice(val(0.05))).eval()){
			...
			fork(new P23Side23(),new P23Side23(),val(0.9));
		}
