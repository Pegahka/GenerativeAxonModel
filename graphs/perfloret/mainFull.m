...
%    full_result = results_full.(divId{i}){1};
    ...
    for param_ind = 1:size(gaResult,1)
        ...
        gen_distributions.(['Solution_' num2str(param_ind)]).Statistics=[gen_data(:,9) gen_data(:,1:8)];
        %% plots        
        ...
        export_fig(filename_rel_path_fig,'-nofontswap');
        clf('reset');
    end    
end
